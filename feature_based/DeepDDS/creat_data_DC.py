...
def creat_data(datafile, cellfile):
    ...
    compound_iso_smiles = []
    df = pd.read_csv('data/drugcomb/drug_map_with_smiles.csv')
    
    compound_iso_smiles += list(df['processed_smiles'])
    ...
    if ((not os.path.isfile(processed_data_file_train))):
        ...
        drug1, drug2, cell, label = list(df['drug_a']), list(df['drug_b']), list(df['cell']), list(df['synergy'])
...
if __name__ == "__main__":
    ...
    da = ['drugcombdb/filtered_updated_drugcombdb']
    for datafile in da:
